...
int main(int argc, char* argv[]) {
	...
	HANDLE hEnableReadServer = OpenEvent(EVENT_ALL_ACCESS, FALSE, L"EnableReadServer");// для синхронизации обмена данными
	HANDLE hEnableReadSearch = OpenEvent(EVENT_ALL_ACCESS, FALSE, L"EnableReadSearch");// для синхронизации обмена данными
	if (hEnableReadSearch == NULL || hEnableReadServer == NULL) {// открываем событие, разрешающее чтение
	...
	WaitForSingleObject(hEnableReadSearch, INFINITE);// ждем разрешения на чтение
	...
	if (!ReadFile(hReadPipe, arr1, sizeof(__int16) * size1, &byteRead, NULL))
	{
	...
	if (!ReadFile(hReadPipe, arr2, sizeof(__int8) * size2, &byteRead, NULL))
	{
